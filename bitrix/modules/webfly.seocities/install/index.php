...
Class webfly_seocities extends CModule {
    ...
    function InstallFiles($arParams = array()) {
        CopyDirFiles($_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/webfly.seocities/style", $_SERVER["DOCUMENT_ROOT"] . "/bitrix/css/webfly.seocities", true, true);
        return true;
    }

    function UnInstallFiles() {
        DeleteDirFilesEx("/bitrix/components/webfly/meta.edit");
        DeleteDirFilesEx("/bitrix/components/webfly/cities.popup");
        DeleteDirFilesEx("/bitrix/css/webfly.seocities");
        return true;
    }
    ...
    function DoInstall() {
        ...
        global $DB;
        //Update ReplaceRegionMacros sorting
        $rsEvents = GetModuleEvents("main", "OnEndBufferContent");
        $tableUpdate = "b_module_to_module";
        $toMethod = "ReplaceRegionMacros";

        while ($arEvent = $rsEvents->Fetch()) {
            if ($arEvent["TO_METHOD"] == $toMethod)
                $RRMHandler = $arEvent;
        }
        if ($RRMHandler) {
            $DB->PrepareFields($tableUpdate);
            $dbFields = array("SORT" => 150);
            $DB->StartTransaction();
            $DB->Update($tableUpdate, $dbFields, "WHERE TO_METHOD='" . $toMethod . "'", $err_mess . __LINE__);
            if ($err_mess)
                $DB->Rollback();
            else
                $DB->Commit();
