	...
	CFormInfo.prototype.PHPParser = function (str)
	{
		...
		if (func)
		{
			if (func.name.substr(0, 7) == '$FORM->')
			{
				...
				{
					case 'ShowInput':
						var __par_FIELD_SID = oBXEditorUtils.PHPParser.trimQuotes(func.params);
						...
						if (element_index !== false)
						{
							...
							var funcstr = '<img id="' + pMainObj.SetBxTag(false, {tag: "form_question", params: {name:'element_index', value:element_index}}) + '" src="/bitrix/images/form/visual/form_edit_' + __q_obj.type + '.gif"  border="0" />';
							return funcstr;
						}
						else
						{
					...
					default:
						...
						var funcparams = {
							name: 'METHOD',
							value: func.name,
							'1': {name:'INDEX', value:_index}
						};

						if (func.name.toLowerCase() == 'showsubmitbutton' || func.name.toLowerCase() == 'showapplybutton' || func.name.toLowerCase() == 'showresetbutton')
						{
							var funcstr = '<img id="' + pMainObj.SetBxTag(false, {tag: "form_api", params: funcparams}) + '" src="/bitrix/images/form/visual/form_api_edit_' + func.name.toLowerCase() + '.gif" border="0" />';
						}
						else
						{
							var funcstr = '<img id="' + pMainObj.SetBxTag(false, {tag: "form_api", params: funcparams}) + '" src="/bitrix/images/form/visual/form_api_' + func.name.toLowerCase() + '.gif" border="0" />';
						}
						return funcstr;
				}
			}
